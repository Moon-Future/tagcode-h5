...
const portraitCover = computed(() => Boolean(coverImage.value?.width && coverImage.value?.height && coverImage.value.height > coverImage.value.width))
  ...
  <article class="custom-detail">
    <header class="custom-hero">
      <template v-if="coverImage">
        <img v-if="portraitCover" class="custom-hero-backdrop" :src="mediaUrl(apiBase, coverImage)" alt="">
        <img :class="['custom-hero-image',{portrait:portraitCover}]" :src="mediaUrl(apiBase, coverImage)" :alt="name" @click="previewIndex = 0">
      </template>

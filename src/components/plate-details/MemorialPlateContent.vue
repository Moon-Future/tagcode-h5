...
<template>
  <article class="memorial-detail">
    <header class="memorial-hero">
      <img v-if="photos[0]" :src="mediaUrl(apiBase, photos[0])" :alt="title" @click="openPreview(0)">
...
.memorial-hero{position:relative;height:auto;aspect-ratio:16/10;min-height:0;overflow:hidden;background:#d7c4b2;clip-path:ellipse(160% 100% at 50% 0);-webkit-clip-path:ellipse(160% 100% at 50% 0)}.memorial-hero>img{display:block;width:100%;height:100%;object-fit:cover}.memorial-hero-shade{position:absolute;inset:0;pointer-events:none;background:linear-gradient(180deg,rgba(34,23,17,.05),rgba(34,23,17,0) 78%,rgba(34,23,17,.04))}.memorial-fallback{display:grid;width:100%;height:100%;place-items:center;background:linear-gradient(145deg,#e8c4b1,#b98975);color:rgba(255,255,255,.7)}
...
.memorial-gallery-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.memorial-gallery-grid button{aspect-ratio:16/10;padding:0;overflow:hidden;border:0;border-radius:11px;background:#eadfd7;cursor:zoom-in}.memorial-gallery-grid img{display:block;width:100%;height:100%;object-fit:cover}.memorial-gallery-grid button:last-child:nth-child(odd){grid-column:1/-1}
...
.memorial-hero{aspect-ratio:4/3}.memorial-hero>img{object-fit:contain}.memorial-gallery-grid{display:flex;flex-direction:column}.memorial-gallery-grid button{width:100%;aspect-ratio:4/3}.memorial-gallery-grid img{object-fit:contain}
@media (max-width:380px){.memorial-heading h1{font-size:28px}}
</style>
